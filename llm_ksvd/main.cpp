...
int main(int argc, char **argv)
{
	...
	if (argc < 11)
	{
		cout << "usage: K-SVD image add_noise sigma noisy denoised difference bias diff_bias \
	...
	if (add_noise) {
		...
		for (unsigned k = 0; k < whc; k++)
			img_noisy[k] = (double) (img[k] * 255.0l);
		}
	cout << "done." << endl; 


	//! Denoising
	bool useAcceleration = atof(argv[10]);
	...
	if (add_noise) {
		if (io_png_write_f32(argv[4], img_noisy_f, width, height, chnls) != 0)
			cout << "... failed to save png image " << argv[4] << endl;	}

	if (io_png_write_f32(argv[5], img_denoised, width, height, chnls) != 0)
		cout << "... failed to save png image " << argv[5] << endl;

    if (io_png_write_f32(argv[6], img_diff, width, height, chnls) != 0)
		cout << "... failed to save png image " << argv[6] << endl;
    if (atof(argv[9]))
    {
        if (io_png_write_f32(argv[7], img_bias, width, height, chnls) != 0)
            cout << "... failed to save png image " << argv[7] << endl;

        if (io_png_write_f32(argv[8], img_diff_bias, width, height, chnls) != 0)
            cout << "... failed to save png image " << argv[8] << endl;
    }
    cout << "done." << endl;
