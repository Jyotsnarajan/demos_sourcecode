"""
parameter-free fast pixelwise non-local-means denoising ipol demo web app
"""
...
class app(base_app):
    """ Parameter-Free Fast Pixelwise Non-Local Means Denoising app """

    title = "Parameter-Free Fast Pixelwise Non-Local Means Denoising"
    xlink_article = 'http://www.ipol.im/pub/pre/120/'
    ...
    def build(self):
        ...
        """
        version = 1.2
        name = "nlmp_%s" % ((version))
        tgz_name = name + ".tar.gz"
        url = "http://www.ipol.im/pub/art/2014/120/"
        tgz_url = url + name + ".tgz"
        tgz_file = self.dl_dir + tgz_name
        progs = ["NLMeansP", "img_mse_ipol", "img_diff_ipol"]
        src_bin = dict([(self.src_dir + os.path.join(name, prog),
                         self.bin_dir + prog)
                        for prog in progs])
        ...
        if all([(os.path.isfile(bin_file)
            ...
            # build the programs
            build.run("make -C %s %s opti OMP=1"
                      % (self.src_dir + name, " ".join(progs)),
                      stdout=log_file)
            # save into bin dir
    ...
    def run_algo(self, sigma, stdout=None, timeout=False):
        ...
        p = self.run_proc(['NLMeansP', 'input_0.sel.png',
