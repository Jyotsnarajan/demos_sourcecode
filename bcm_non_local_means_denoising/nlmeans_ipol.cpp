...
// usage: nlmeans_ipol image add_noise sigma noisy denoised

int main(int argc, char **argv) {


    if (argc < 6) {
        printf("usage: nlmeans_ipol image add_noise sigma noisy denoised \n");
        exit(-1);
    }
	...
	if (add_noise == 1) {
		...
		if (io_png_write_f32(argv[5], noisy, (size_t) d_w, (size_t) d_h, (size_t) d_c) != 0) {
			printf("... failed to save png image %s", argv[5]); }
		}	
